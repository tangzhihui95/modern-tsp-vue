    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleSort"
          v-hasPermi="['system:post:sort']"
        >岗位排序</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="postList" @selection-change="handleSelectionChange" row-key="postId">
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除岗位编号为"' + postIds + '"的数据项？').then(function() {
        return delPost(postIds).then(res=>{
          that.getList();
          if(res.errInfo.errcode===1){
            that.$message.error(res.errInfo.errmsg);
          }else{
    ...
    handleSort() {
      if (this.queryParams.postSort === "") {
        this.queryParams.postSort = "0";
      }
      else if (this.queryParams.postSort === "0") {
        this.queryParams.postSort = "1";
      }
      else if (this.queryParams.postSort === "1") {
        this.queryParams.postSort = "0";
      }
      this.getList();
    },
